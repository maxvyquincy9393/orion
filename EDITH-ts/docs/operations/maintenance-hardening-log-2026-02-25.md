...
- Added Phase-1 global-style `nova` CLI wrapper (`bin/nova.js`) with OpenClaw-like command ergonomics:
  - `nova link <repo>`
  - `nova quickstart`
  - `nova wa scan`
  - `nova wa cloud`
  - `nova all`, `nova doctor`, `nova gateway`
- Wrapper stores linked repo path in `~/.nova/cli.json` and proxies to `pnpm --dir <repo> ...`.
- Added `package.json` bin entry for npm global install (`nova`).
- Added helper tests for CLI parsing/repo detection and docs:
  - `src/cli/__tests__/nova-global.test.ts`
  - `docs/platform/global-cli.md`
- Validation:
  - `node bin/nova.js --help` prints expected OpenClaw-style wrapper commands
  - `node bin/nova.js repo --repo .` resolves current repo path
  - `pnpm typecheck` passes
  - `pnpm test:ci` => `21` test files passed / `73` tests passed

## Follow-up Notes (pass 19)

- Phase-2 global wrapper improvements (`bin/nova.js`):
  - added profile bootstrap + runtime env forwarding (`NOVA_ENV_FILE`, `NOVA_WORKSPACE`, `NOVA_STATE_DIR`)
  - new commands: `nova profile`, `nova profile init`, `nova init`
  - wrapper now auto-bootstraps profile env/workspace/state before running Nova commands
- `src/config.ts` now honors `NOVA_ENV_FILE` so onboarding/runtime can use profile-scoped `.env`.
- `src/cli/onboard.ts` now writes to `NOVA_ENV_FILE` target when provided (global wrapper path).
- `src/channels/whatsapp.ts` now stores Baileys auth under `NOVA_STATE_DIR` (profile-scoped) when set.
...
- Validation:
  ...
  - `node bin/nova.js profile init --repo .` creates `%USERPROFILE%\\.nova\\profiles\\default` (`.env`, `workspace`, `.nova` state dir)
...
- Added `nova self-test` (beginner-friendly readiness report) to global wrapper:
...
- Validation:
  ...
  - `node bin/nova.js self-test --repo . --profile .tmp-nova-profile` prints actionable report (sandbox: `EPERM`; escalated run reports `ENOENT` when `pnpm` is not on PATH in that shell)

## Follow-up Notes (pass 21)

- Global `nova` CLI Windows hardening + bug fixes from real end-to-end testing:
  - fixed Windows direct-execution detection for npm shim/symlink/casing edge cases (`nova --help` no longer silently exits)
  - wrapper now uses `pnpm.cmd` on Windows and plain `pnpm` elsewhere
  - wrapper/self-test spawn helpers now enable `shell` only for Windows `.cmd/.bat` commands (fixes false `pnpm` failure in `nova self-test`)
  - `nova profile init --repo ... --profile ...` and `nova init` with overrides no longer force-write `~/.nova/cli.json` (override stays per-command)
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (Windows invoke path, `pnpm.cmd`, shell-for-cmd rule)
- Global CLI flow verified on Windows:
  - `npm install -g .`
  - `nova --help`
  - `nova repo`
  - `nova profile init --repo ... --profile .tmp-nova-profile`
  - `nova self-test --repo ... --profile .tmp-nova-profile` (outside sandbox: `OK pnpm`)
  - `nova doctor --repo ... --profile .tmp-nova-profile`
  - `nova wa scan --repo ... --profile .tmp-nova-profile` starts WhatsApp QR wizard and shows provider prompt (interactive smoke)
- Validation:
  - `pnpm typecheck` passes
...
- Improved beginner/non-interactive WhatsApp QR onboarding for OpenClaw-style global CLI usage:
  - `nova wa scan --yes --provider groq` is now truly non-interactive (skips optional prompts, uses defaults, writes env directly).
  - global wrapper now forwards extra args after `nova wa scan` / `nova wa cloud` to underlying onboarding scripts.
- Fixed onboarding UX mismatch when called from global `nova` wrapper:
  - next-step instructions now suggest `nova doctor` / `nova all` / `nova onboard` when wrapper env is detected.
...
- Added tests/docs:
  ...
  - `docs/platform/global-cli.md` (scriptable `nova wa scan --yes --provider groq` example).
...
- Validation:
  ...
  - real command smoke (outside sandbox): `nova wa scan --yes --provider groq --repo ... --profile .tmp-nova-profile` exits 0 and writes WhatsApp QR config without prompts
...
- OpenClaw-alignment tranche 1 + 2 (CLI surface + flag parity):
  - added wrapper command surface parity aliases:
    - `nova setup`, `nova configure`, `nova dashboard`, `nova status`, `nova logs`
  - `nova status` now aliases readiness/self-test flow (OpenClaw-like status entrypoint)
  - `nova logs` provides a foreground live-log fallback (`all` / `gateway`) and clear guidance when daemon-style log storage is unavailable
  - `nova dashboard` prints dashboard URL (`http://127.0.0.1:<gateway-port>`) then starts gateway foreground mode
- Added global flag parity primitives:
  - `--profile <name>` now maps to `~/.nova/profiles/<name>` (while explicit paths still work)
  - `--dev` uses isolated `~/.nova/profiles/dev` profile
  - `nova quickstart/setup/configure/init` now forward extra onboarding args (e.g. `--non-interactive`, `--provider`, `--channel`)
...
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (`--dev`, profile-name mapping, profile selector path/tilde handling)
...
- Validation:
  ...
  - real command smoke:
    - `nova --help`
    - `nova profile --repo ... --profile work` => resolves `~/.nova/profiles/work`
    - `nova --dev profile --repo ...` => resolves `~/.nova/profiles/dev`
    - `nova setup --non-interactive --provider groq --channel whatsapp --whatsapp-mode scan --repo ... --profile .tmp-nova-profile`
    - `nova status --repo ... --profile .tmp-nova-profile`
    - `nova logs foo --repo ... --profile .tmp-nova-profile` (guidance path)

## Follow-up Notes (pass 24)

- OpenClaw-alignment tranche 3 (channels namespace facade):
  - added `nova channels help|login|status|logs` namespace commands to global wrapper
  - `nova channels login --channel whatsapp` maps to existing WhatsApp setup flow (`wa:scan` by default, `--mode cloud` supported)
  - `nova channels status` currently reuses readiness/self-test (best available status surface today)
  - `nova channels logs` currently reuses live foreground Nova logs / guidance path (no daemon log store yet)
...
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (`parseChannelsArgs`, channel/mode normalization)
- Documentation updates:
...
- Validation:
  ...
  - real command smoke:
    - `nova channels help`
    - `nova channels login --channel whatsapp --non-interactive --provider groq --repo ... --profile .tmp-nova-profile`
    - `nova channels status --channel whatsapp --repo ... --profile .tmp-nova-profile`
    - `nova channels logs foo --repo ... --profile .tmp-nova-profile` (guidance path)

## Follow-up Notes (pass 25)

- OpenClaw-alignment tranche 4a (readiness polish for global CLI):
  - added `nova self-test --fix` safe fix mode:
  ...
  - `nova status --fix` now behaves the same as `nova self-test --fix`
- Improved `nova channels status`:
  - `nova channels status --channel <name>` now prints channel-focused readiness checks for:
    - `whatsapp`
    - `telegram`
    - `discord`
    - `webchat`
  - channel status now exits non-zero when it reports channel errors (matching `nova self-test` exit-code behavior)
- Improved `nova channels logs` UX:
...
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (`parseSelfTestArgs`, Telegram/Discord/WebChat channel checks)
- Documentation updates:
...
- Validation:
  ...
  - local `nova all` smoke remains blocked in sandbox by `tsx/esbuild spawn EPERM` (environment limitation)

## Follow-up Notes (pass 27)

- OpenClaw-style CLI focus (first-run entrypoint UX):
  - bare `nova` now acts as a smart entrypoint instead of always printing help
  - if no linked repo exists but command is run inside/near an Nova repo, wrapper auto-detects and auto-links it
  - if the active profile is not configured (no provider/channel setup), bare `nova` launches the setup wizard automatically (`quickstart`)
  - if profile is already configured, bare `nova` prints concise next actions (`dashboard`, `channels login`, `all`, `status`)
- CLI startup hardening for fresh profiles:
  - `nova all` and `nova gateway` now auto-run profile-scoped `prisma migrate deploy` preflight before starting
  - uses the active profile `DATABASE_URL` so fresh profile DBs do not fail with `P2021` missing-table errors on first run
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (`isProfileEnvLikelyConfigured` helper for smart entrypoint heuristics)
- Documentation updates:
  - `docs/platform/global-cli.md` (bare `nova` smart entrypoint + auto-migrate preflight notes)
...
- CLI support/automation parity improvements:
  - added `--json` output mode for:
    - `nova self-test`
    - `nova status`
    - `nova channels status --channel <name>`
  - added `--migrate` option to `nova self-test` / `nova status`
...
- Channel status behavior:
  - `nova channels status --json` now forwards JSON mode to global self-test when no explicit channel is selected
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (`parseSelfTestArgs` and `parseChannelsArgs` coverage for `--migrate` / `--json`)
- Documentation updates:
...
- OpenClaw-style dashboard-first CLI polish:
  - added `nova dashboard --open` / `--no-open`
  - `--open` best-effort opens the dashboard URL in the default browser, then starts gateway foreground mode
  - `nova dashboard --help` now documents dashboard-specific flags
- Added tests:
  - `src/cli/__tests__/nova-global.test.ts` (`parseDashboardArgs`)
- Documentation updates:
  - `docs/platform/global-cli.md` (`nova dashboard --open` examples)

## Follow-up Notes (pass 30)

- OpenClaw-style CLI status parity (channel runtime hints):
  - `nova channels status --channel whatsapp` now augments readiness checks with runtime auth-state inspection for Baileys QR mode:
...
- Help text polish:
  - `nova channels help` now clarifies `channels status --channel ...` is channel-focused status with runtime hints where supported, while bare `channels status` remains global self-test
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (WhatsApp auth-state inspection + creds summary helpers)

## Follow-up Notes (pass 31)

- OpenClaw-style CLI status parity expanded beyond WhatsApp:
  - `nova channels status --channel telegram` now includes token-format sanity hints + masked preview in runtime JSON (best-effort, no network call)
  - `nova channels status --channel discord` now includes token-format sanity hints + masked preview in runtime JSON (best-effort, no network call)
  - `nova channels status --channel webchat` now probes localhost WebChat port reachability and reports runtime probe results (`reachable`, `latencyMs`, error)
...
- Added/extended tests:
  - `src/cli/__tests__/nova-global.test.ts` (Telegram/Discord token summaries + local TCP probe helper)

## Follow-up Notes (pass 32)

- OpenClaw-style `channels logs` parity improvement:
  - `nova channels logs --channel <name>` now runs live foreground logs with best-effort line filtering instead of always falling back to unfiltered global logs
  - supported filters:
...
- Help/docs polish:
  - `nova channels help` now describes channel-filtered logs behavior
  - `docs/platform/global-cli.md` and `docs/channels/whatsapp.md` updated accordingly
- Added tests:
  - `src/cli/__tests__/nova-global.test.ts` (`lineMatchesChannelLogFilter` coverage for channel tags + fatal passthrough)

## Follow-up Notes (pass 33)

- `channels logs` UX hardening (user-feedback driven):
  - `nova logs` and `nova channels logs --channel <name>` now run profile DB migration preflight before starting foreground logs (same safety behavior as `nova all` / `nova gateway`)
  ...
  - channel-filtered logs now emit one-time actionable hints for common patterns:
    - Prisma missing-table errors (`P2021`) -> suggest `nova status --fix --migrate`
...
- Added tests:
  - `src/cli/__tests__/nova-global.test.ts` (`getChannelLogHints` coverage for `P2021` + WhatsApp 405`)
