# Nova — Research Papers: Gap Features vs OpenClaw
...
- **Untuk Nova**: `src/engine/orchestrator.ts` perlu upgrade dari flat priority list → difficulty-aware routing. Track latency histogram per engine + per task type, dynamically select engine berdasarkan historical performance.
...
- **Mapping ke Nova**: Setiap incoming task → estimate difficulty (1-5) → route ke appropriate engine tier:
...
- **Untuk Nova (future)**: Supervisor agent dalam `runner.ts` bisa evolve menjadi Conductor-style — dynamically compose subagent topology berdasarkan task structure.
...
- **Untuk Nova**: `src/core/loop-detector.ts` (file baru) — monitor tool call history per session, detect patterns, inject WARNING atau circuit-break
...
- **Untuk Nova**: Sebelum loop terjadi, monitor DQ-like metric per turn. Jika output quality degrades → pre-emptive intervention.
...
- **Untuk Nova**: Upgrade `session-summarizer.ts` → tambah auto-trigger ketika context > 70% full. Tambah failure hindsight notes di `memory/hindsight/YYYY-MM-DD.md`
...
- **Untuk Nova**: `src/memory/compaction-manager.ts` — tiered retention dengan auto-trigger + verifier pass setelah compaction
...
- **Key findings**:
  ...
  - Nova sekarang = "Assisted Agent" level. Dengan exec approvals → naik ke "Collaborative Agent"
- **Design pattern**: Interrupt → notify user → wait → resume (bukan block → reject)
- **Untuk Nova**: `src/security/approval-gate.ts` — classify tool risk level (read-only / reversible write / irreversible write), request approval via channel (WhatsApp/WebChat) untuk irreversible actions
...
- **Untuk Nova**: `src/security/approval-gate.ts` dengan async approval pattern, pending approvals disimpan di `memory/approvals/pending.json`
...
- **Untuk Nova**: Extend `causal-graph.ts` dengan opinion evolution layer. Ketika task fail → CARA-style reflection → extract "what went wrong" → inject ke procedural memory sebagai hindsight note.
- **Critical gap yang di-fix**: Current Nova treats memory sebagai external retrieval only — tidak ada separation antara evidence vs inference. Hindsight pattern fix this.
...
- **Untuk Nova**: Foundation untuk upgrade `himes.ts` → full memory OS layer
...
- **Untuk Nova**: MEMORY.md (Vault) bisa di-extend: agent proactively research topics yang sering muncul → store sebagai semantic knowledge

---

## SUMMARY TABLE: Paper → Gap Feature Mapping

| Paper | arXiv | Gap Feature | Target File di Nova |
|---|---|---|---|
...
Setelah ini semua done → Nova feature parity dengan OpenClaw production.
