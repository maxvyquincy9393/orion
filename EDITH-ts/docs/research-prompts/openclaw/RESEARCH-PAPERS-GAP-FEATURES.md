# EDITH — Research Papers: Gap Features vs EDITH
...
- **Untuk EDITH**: `src/engine/orchestrator.ts` perlu upgrade dari flat priority list → difficulty-aware routing. Track latency histogram per engine + per task type, dynamically select engine berdasarkan historical performance.
...
- **Mapping ke EDITH**: Setiap incoming task → estimate difficulty (1-5) → route ke appropriate engine tier:
...
- **Untuk EDITH (future)**: Supervisor agent dalam `runner.ts` bisa evolve menjadi Conductor-style — dynamically compose subagent topology berdasarkan task structure.
...
- **Untuk EDITH**: `src/core/loop-detector.ts` (file baru) — monitor tool call history per session, detect patterns, inject WARNING atau circuit-break
...
- **Untuk EDITH**: Sebelum loop terjadi, monitor DQ-like metric per turn. Jika output quality degrades → pre-emptive intervention.
...
- **Untuk EDITH**: Upgrade `session-summarizer.ts` → tambah auto-trigger ketika context > 70% full. Tambah failure hindsight notes di `memory/hindsight/YYYY-MM-DD.md`
...
- **Untuk EDITH**: `src/memory/compaction-manager.ts` — tiered retention dengan auto-trigger + verifier pass setelah compaction
...
- **Key findings**:
  ...
  - EDITH sekarang = "Assisted Agent" level. Dengan exec approvals → naik ke "Collaborative Agent"
- **Design pattern**: Interrupt → notify user → wait → resume (bukan block → reject)
- **Untuk EDITH**: `src/security/approval-gate.ts` — classify tool risk level (read-only / reversible write / irreversible write), request approval via channel (WhatsApp/WebChat) untuk irreversible actions
...
- **Untuk EDITH**: `src/security/approval-gate.ts` dengan async approval pattern, pending approvals disimpan di `memory/approvals/pending.json`
...
- **Untuk EDITH**: Extend `causal-graph.ts` dengan opinion evolution layer. Ketika task fail → CARA-style reflection → extract "what went wrong" → inject ke procedural memory sebagai hindsight note.
- **Critical gap yang di-fix**: Current EDITH treats memory sebagai external retrieval only — tidak ada separation antara evidence vs inference. Hindsight pattern fix this.
...
- **Untuk EDITH**: Foundation untuk upgrade `himes.ts` → full memory OS layer
...
- **Untuk EDITH**: MEMORY.md (Vault) bisa di-extend: agent proactively research topics yang sering muncul → store sebagai semantic knowledge

---

## SUMMARY TABLE: Paper → Gap Feature Mapping

| Paper | arXiv | Gap Feature | Target File di EDITH |
|---|---|---|---|
...
Setelah ini semua done → EDITH feature parity dengan EDITH production.
