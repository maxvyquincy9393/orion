...
// Phase H: OS-Agent perception
import type { OSAgent } from "../os-agent/index.js"
...
/**
 * Phase H: Build a perception context section from OS-Agent's
 * active context snapshot — enables the LLM to know what the user
...
function buildPerceptionSection(): string | null {
  ...
  try {
    const summary = osAgent.perception.summarize()
    if (!summary || summary.trim().length === 0) return null
    return `# Active OS Context (Live)\n${summary}`
  } catch {
    return null
  }
}
...
export async function buildSystemPrompt(options: BuildPromptOptions = {}): Promise<string> {
  ...
  // Phase H: Inject OS-Agent perception context
  const perception = buildPerceptionSection()
  if (perception) {
