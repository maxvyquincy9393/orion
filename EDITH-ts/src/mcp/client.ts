 ...
 *
 * Connects Nova to any MCP server and dynamically registers their
 * tools into the Nova tool registry at runtime.
 *
 * Architecture (MCP-Zero, arXiv 2506.01056):
 *   - Static tool registry: tools configured in nova.json
...
export interface MCPServerConfig {
  ...
  /** Whether this server starts automatically on Nova startup */
  autoStart?: boolean
}
 ...
 * their tools as Nova-compatible tool definitions.
 */
export class MCPClientManager {
  ...
  /**
   * Load MCP server configs from nova.json and connect to autoStart servers.
   */
  ...
  private async connectHttp(conn: MCPConnection): Promise<void> {
    ...
    const response = await fetch(`${conn.config.url}/initialize`, {
      ...
      body: JSON.stringify({
        ...
        id: ++this.requestIdCounter,
        method: "initialize",
        params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "nova", version: "1.0.0" } },
      }),
    })
