...
/**
 * Temporal decay parameters for causal edges.
 *
 * Edges that haven't been reinforced (low evidence) decay faster than
 * well-established causal links. Uses an exponential decay model:
 *
 *   effectiveStrength = strength × exp(-daysSinceCreation / (halfLifeDays × (1 + ln(evidence))))
 *
 * The evidence term acts as a stabilizer: an edge seen once (evidence=1)
 * decays with the base halfLife (90 days), while an edge reinforced 10 times
 * takes ~3× longer to decay by half.
 *
 * Ref: Ebbinghaus forgetting curve + spacing effect (arXiv 2601.06152 §3.4)
 */
const EDGE_DECAY_HALF_LIFE_DAYS = 90
const MS_PER_DAY = 86_400_000

function applyTemporalEdgeDecay(
  strength: number,
  evidence: number,
  createdAt: Date,
  now: Date = new Date(),
  ...
  const stabilizer = 1 + Math.log(Math.max(1, evidence))
  const effectiveHalfLife = EDGE_DECAY_HALF_LIFE_DAYS * stabilizer
  const decayFactor = Math.exp((-ageDays * Math.LN2) / effectiveHalfLife)

  return clamp(strength * decayFactor)
}
  ...
  async getDownstreamEffects(
    ...
    try {
      ...
      return edges.map((edge) => ({
        ...
        strength: config.CAUSAL_EDGE_DECAY_ENABLED
          ? applyTemporalEdgeDecay(edge.strength, edge.evidence, edge.createdAt)
          : edge.strength,
    ...
    try {
      ...
      while (queue.length > 0) {
        ...
        for (const edge of edges) {
          ...
          const effectiveStrength = config.CAUSAL_EDGE_DECAY_ENABLED
            ? applyTemporalEdgeDecay(edge.strength, edge.evidence, edge.createdAt)
            : edge.strength
