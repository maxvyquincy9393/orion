...
- same Orion pipeline path used by web/Telegram/Discord
...
- Orion webhook ingestion parses messages quickly
...
- `/id`, `!id`
- `/ping`, `!ping`
- explicit failure reply if Orion pipeline processing fails
...
WHATSAPP_CLOUD_VERIFY_TOKEN=orion-wh-verify-123
...
3. Run Orion with gateway + channels:
   - `pnpm all`
...
- Messages arrive but no Orion reply:
