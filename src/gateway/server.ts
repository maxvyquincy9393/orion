  ...
  private registerRoutes(): void {
    ...
    this.app.register(async (app) => {
      ...
      app.post<{
        ...
        // Lazy import to avoid circular dependency at boot
      ...
      // ── Phase 16: Mobile Background Sync Delta ───────────────────────

      /**
       * GET /api/sync/delta
       * Polled by the mobile background task for battery-efficient sync.
       * Returns only changed data since `since` timestamp.
       *
       * Query params:
       *   since  - ISO timestamp of last successful sync (default: last 24 h)
       */
      app.get<{ Querystring: { since?: string } }>(
        ...
        async (req, reply) => {
          ...
          const userId = auth.userId
          // sinceStr stored for potential future use in session-delta filtering
          const _sinceDate = req.query.since
            ? new Date(req.query.since)
            : new Date(Date.now() - 24 * 60 * 60 * 1000)
          void _sinceDate // acknowledged: will be used when session-delta API is wired

          // Fetch recent memory context and upcoming calendar alerts in parallel
